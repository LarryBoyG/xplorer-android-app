...
/* package */ final class RtspMessageChannel implements Closeable {
  ...
  private static final class MessageParser {
    ...
    private ImmutableList<String> addMessageBody(byte[] messageBodyBytes) {
      ...
      int messageBodyLength = messageBodyBytes.length;
      if (messageBodyLength > 0 && messageBodyBytes[messageBodyLength - 1] == Ascii.LF) {
        if (messageBodyLength > 1 && messageBodyBytes[messageBodyLength - 2] == Ascii.CR) {
          // Line ends with CRLF.
          messageBodyLength -= 2;
        } else {
          // Line ends with LF.
      ...
      // RTSP bodies are length-delimited and are not required to end with CRLF/LF. XIRO cameras
      // return fixed-length GET_PARAMETER bodies like "2013.07.03" without a trailing newline.
      String messageBody =
          new String(messageBodyBytes, /* offset= */ 0, /* length= */ messageBodyLength, CHARSET);
