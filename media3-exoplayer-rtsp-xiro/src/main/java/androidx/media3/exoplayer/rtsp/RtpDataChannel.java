...
/* package */ interface RtpDataChannel extends DataSource {
  ...
  /**
   * Optionally primes the transport path once the RTSP server announces the destination RTP port.
   *
   * <p>Most implementations do nothing. XIRO legacy UDP playback uses this hook to send the small
   * client-side UDP punch packets the original app emits through the range extender path.
   */
  default void primeTransportPath(String host, int port) {}
