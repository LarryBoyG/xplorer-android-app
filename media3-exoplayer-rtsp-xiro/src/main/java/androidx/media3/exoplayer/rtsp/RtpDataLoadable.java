...
/* package */ final class RtpDataLoadable implements Loader.Loadable {
  ...
  private static final long XIRO_LEGACY_UDP_STARTUP_TIMEOUT_MS = 7_500L;
  ...
  public void load() throws IOException {
    ...
    try {
      ...
      while (!loadCancelled) {
        ...
        if (shouldCompleteUdpStartupForTcpFallback(loadStartedAtMs)) {
          Log.d(
              TAG,
              "No RTP packets arrived on UDP track "
                  + trackId
                  + " within the legacy startup timeout; completing load to trigger TCP retry.");
          DataSourceUtil.closeQuietly(checkNotNull(dataChannel));
          break;
        }
      }
      ...
      @Nullable RtpDataChannel finalDataChannel = dataChannel;
      if (finalDataChannel != null && finalDataChannel.needsClosingOnLoadCompletion()) {
        DataSourceUtil.closeQuietly(finalDataChannel);
        if (dataChannel == finalDataChannel) {
          dataChannel = null;
        }
      }
    }
  ...
  private boolean shouldCompleteUdpStartupForTcpFallback(long loadStartedAtMs) {
    @Nullable RtpDataChannel currentDataChannel = dataChannel;
    if (currentDataChannel == null) {
      return false;
    }
    if (currentDataChannel.getInterleavedBinaryDataListener() != null) {
      return false;
    }
    if (checkNotNull(extractor).hasReadFirstRtpPacket()) {
      return false;
    }
    return SystemClock.elapsedRealtime() - loadStartedAtMs >= XIRO_LEGACY_UDP_STARTUP_TIMEOUT_MS;
  }
  ...
  private void closeCurrentDataChannel() {
    @Nullable RtpDataChannel currentDataChannel = dataChannel;
    if (currentDataChannel != null) {
      DataSourceUtil.closeQuietly(currentDataChannel);
      if (dataChannel == currentDataChannel) {
        dataChannel = null;
      }
    }
  }
